...
class HBOSPYOD(BaseDetector):
    def __init__(self, mode="static", n_bins="auto", adjust=False, save_scores=False, log_scale=True, ranked=False,
    ...
    def fit(self, X, y=None):
        ...
        if self.mode == "static":
            ...
            self.decision_scores_ = self.calc_hbos_scores(self.samples, self.features, self.bin_id_array)
    ...
    '''def calc_hbos_scores_npsum(self, samples, features, bin_id_array):
        self.score_array=np.array(self.score_array)
        bin_id_array=np.array(bin_id_array)
        ...
        for b in range(features):
            ...
            highestid = self.highest_score_id[b]
            highest_score = scores_b[highestid - 1]
            self.score_array[b] = [score if score != 0 else highest_score for score in self.score_array[b]]
            if self.log_scale:
                self.score_array[b]=np.log(self.score_array[b])
        if self.ranked:
            self.rank_scores_no_empty_bins()
        hbos_scores = np.zeros(samples)
        for c in range(samples):
            bin_scores = [self.score_array[b][bin_id_array[b][c] - 1] for b in range(features)]
            hbos_scores[c] = np.sum(bin_scores)
        return hbos_scores'''
    ...
    def calc_hbos_scores(self, samples, features, bin_id_array):
        ...
        for i in range(samples):
            ...
            if self.save_scores:
                self.explainability_scores.append(scores_per_sample)
            hbos_scores[i] = score
        return hbos_scores
    ...
    def get_explainability_scores(self,sampleid):
        if sampleid == "all":
            return self.explainability_scores
        else:
            return self.explainability_scores[sampleid]
