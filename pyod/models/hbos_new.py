...
class HBOSPYOD(BaseDetector):
    ...
    (with a few minor exceptions) and the bin width can vary.
    ...
    A ranked mode is also supported, in which the histogram get sorted by score.
    Rank of the bins => new score
    ...
    smoothen : bool, optional (default=False)
        ...
        the neighbour bins is taken. See get_bin_density_smooth()
    ...
    bin_width_array_: array of shape (n_bins, n_features)
        The bin widths of the bins, needed to calculate the score.
    ...
    highest_score_id_ : array of shape (n_features)
        The ids of the bin with the highest normalized score for each feature.

    highest_score_ : array of shape (n_features)
        The highest score for each feature. (needed to normalize scores)
    ...
    def __init__(self, mode="static", n_bins="auto", samples_per_bin="auto", smooth=False,
    ...
    def get_bin_density(self):
        ...
        Save maximum score for every feature (needed to normalize score)
        """
        for i in range(self.n_features_):  # get the highest data value
            ...
            for j in range(self.n_bins_array_[i] - 1):
                ...
                score = (hist_[j + 1]) / (
                        binwidth * self.n_samples_ / (abs(max_)))  # Bin Höhe / (Bin Breite / höchste bin im Histogramm)
